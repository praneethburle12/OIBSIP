// Register the plugin (Required for GSAP v3) - Wrapped in try-catch for resilience
try {
    if (typeof MorphSVGPlugin !== 'undefined') {
        gsap.registerPlugin(MorphSVGPlugin);
        console.log("MorphSVGPlugin registered successfully.");
    } else {
        console.warn("MorphSVGPlugin not found. Morphing animations (mouth, shoulders) will be disabled, but basic animation will continue.");
    }
} catch (e) {
...
console.log("DOM Elements:", { armL: !!armL, armR: !!armR, email: !!email, password: !!password });

// Inject arm paths to bypass Thymeleaf rendering issues and ensure full fidelity
console.log("Detecting arm placeholders...");
if (armL) {
    console.log("Injecting armL...");
    armL.innerHTML = `
...
function initLoginForm() {
    ...
    if (isMobileDevice()) {
        ...
        if (showPasswordCheck) showPasswordCheck.checked = true;
        gsap.set(twoFingers, { transformOrigin: "bottom left", rotation: 30, x: -9, y: -2, ease: "power2.inOut" });
    }
}

// Ensure DOM is loaded
// Ensure DOM is loaded - Robust initialization
